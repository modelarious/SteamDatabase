...
class ORMMapper:
    def __init__(
        self,
        statement_creation_facade: StatementCreationFacadePostgres,
    ...
    def create_tables(self, database_interaction_func: Callable[[str], Any]):
        for ORMClass in self.ORMClasses:
            table_create_statement = self.statement_creation.get_create_table_statement(
                ORMClass
            )
    ...
    def insert_game(
        self,
        database_interaction_func: Callable[[str, tuple], Any],
        database_interaction_func_multiple_statements: Callable[[str, tuple], Any],
        game: Game,
    ):
        for ORMClass in self.ORMClasses:
            ...
            if ORMClass.needs_multiple_statements():
                non_commit_insertion_func = (
                    database_interaction_func_multiple_statements
                )
            non_commit_insertion_func(sql, insertion_data)

    def get_all_games(
        self, database_interaction_func: Callable[[str, tuple], Any]
    ) -> List[Game]:
