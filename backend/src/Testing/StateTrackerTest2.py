...
class StateCommunicatorTest(unittest.TestCase):
    def setUp(self):
        ...
        for state in STATES:
            ...
            mockSocketWrapper.send_message = MagicMock(
                name=state + " socketWrapperSendMessageMock"
            )

            self.observedDataStructures[state] = ObservedDataStructure(
                mockSocketWrapper
            )
    ...
    def assert_correct_call_structure_after_adding(self, expectedCallValue, state):
        ...
        calls = [call([]), call([expectedCallValue])]
    ...
    def assert_correct_calls_after_moving_to_next_state(self, expectedCallValue, state):
        ...
        calls = [call([]), call([expectedCallValue]), call([])]
        sentMessageMock.assert_has_calls(calls)
    ...
    def test_partial_match_flow(self):
        ...
        self.assert_correct_calls_after_moving_to_next_state(
            gameTitleOnDisk, UPCOMING_STATE
        )
        self.assert_correct_call_structure_after_adding(
            gameTitleOnDisk, FINDING_NAME_ACTIVE_STATE
        )
        ...
        self.assert_correct_call_structure_after_adding(
            userInputRequiredQueueEntry.toDict(), AWAITING_USER_STATE
        )
        ...
        self.assert_correct_call_structure_after_adding(
            matchQueueEntry.toDict(), QUEUED_FOR_INFO_RETRIEVAL_STATE
        )
        ...
        self.assert_correct_call_structure_after_adding(
            matchQueueEntry.toDict(), INFO_RETRIEVAL_ACTIVE_STATE
        )
