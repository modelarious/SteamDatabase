...
def create_connection():
    ...
    conn = psycopg2.connect(
        database="postgres", user='postgres', password='postgres', host=host, port='5432'
    )
...
per_tag_scoring = df.pivot(index="steam_id", columns="genre_name", values="scoring").fillna(0)
...
steam_id_pairs = [(df.index[i], df.index[j]) for i in range(len(df)) for j in range(i+1, len(df))]
